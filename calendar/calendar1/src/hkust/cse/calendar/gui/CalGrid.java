...
public class CalGrid extends JFrame implements ActionListener {
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		//Today show up                 
        final JLabel timeMachineLabel = new JLabel(today.getTime().toString());
        this.add(timeMachineLabel, BorderLayout.SOUTH);

        javax.swing.Timer refreshTime = new javax.swing.Timer(1000, new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                        ...
                        //if timemachine is enabled
                        if(tm.isEnable())
                        {
                                calendar.set(tm.getYear(), tm.getMonth(), tm.getDate(),
                                       tm.getHourOfDay(), tm.getMinute(), tm.getSecond());
                        }
                        today = new GregorianCalendar(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), 
                                        calendar.get(Calendar.DATE), calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE),
                                        calendar.get(Calendar.SECOND));                         
                        timeMachineLabel.setText(today.getTime().toString());
                        if (tempM != today.get(today.MONTH) || tempD != today.get(today.DATE) ||
		...
		//check notification per minute, in CalGrid.java
		Timer timer = new Timer();
		timer.schedule( new TimerTask() {
		    public void run() {
		    	int i=0;
				Appt[] TodayAppt=GetTodayAppt();
		    	if (TodayAppt!=null)
		    	while(i<TodayAppt.length){
		    		int h4timer=TodayAppt[i].TimeSpan().StartTime().getHours();
		    		int m4timer=TodayAppt[i].TimeSpan().StartTime().getMinutes();
		    		int totalm4timer=h4timer*60+m4timer;
		    		Calendar calendar = Calendar.getInstance();
		    		java.util.Date currenttime = calendar.getTime();
		    		int hc = currenttime.getHours();
		    		int mc = currenttime.getMinutes();
		    		if(tm.isEnable()){
		    			hc = tm.getHourOfDay();
		    			mc = tm.getMinute();
		    		}
		    		int totalmc = hc*60+mc;
		    		System.out.print(TodayAppt[i].getNoti());
		    		if(TodayAppt[i].getNoti()!=-1){
		    		if (totalm4timer-totalmc<=TodayAppt[i].getNoti()){
						JOptionPane.showMessageDialog(null, "Notification: "+TodayAppt[i].getTitle()+" in "+TodayAppt[i].getNoti()+" minutes", "Group42", 1);
						TodayAppt[i].setNoti(-1);
						controller.ManageAppt(TodayAppt[i], 3);
	...
	JMenuBar createMenuBar() {
		...
		mi = (JMenuItem) Access.add(new JMenuItem("Change password"));	//adding a Logout menu button for user to logout
		mi.setMnemonic('L');
		mi.getAccessibleContext().setAccessibleDescription("Change password");
		mi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					CPDialog a=new CPDialog(mCurrUser.ID());
			}
		});
		...
		*/
		...
		mi.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0){
				TimeMachine tmg = new TimeMachine();
			}
		});
		Appmenu.add(mi);
...
//	}
	...
	public boolean check_Appt_On_day(int year, int month, int day){
		...
		Appt[] temp_check = controller.RetrieveAppts(null, K);
		if (temp_check.length > 0){check = true;}
		return check;
	}

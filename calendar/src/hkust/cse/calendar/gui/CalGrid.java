...
public class CalGrid extends JFrame implements ActionListener {
	...
	// newly added start
	private final int[][] kfc = new int[6][7];
	// newly added end
...
//		con.testing();
		...
		leftP.setPreferredSize(new Dimension(500, 300));// top left board

		JLabel textL = new JLabel("Important Days");
		textL.setForeground(Color.red); // top left word
		...
		noteP.add(textL, BorderLayout.NORTH);// TOP LEFT BOARD
		noteP.add(note, BorderLayout.CENTER);// TOP LEFT BOARD

		leftP.add(noteP, BorderLayout.CENTER);// TOP LEFT WORD
		...
		JPanel yearGroup = new JPanel(); // year selection
		...
		tableView = new JTable(dataModel) {
			public TableCellRenderer getCellRenderer(int row, int col) {
				...
				boolean have_Appt = false;
				if (tem.equals("") == false) {
					try {
						int tempDay = Integer.parseInt(tem);
						have_Appt = check_Appt_On_day(currentY, currentM, tempDay);
						if (today.get(Calendar.YEAR) == currentY
								&& today.get(today.MONTH) + 1 == currentM
								&& today.get(today.DAY_OF_MONTH) == Integer
										.parseInt(tem)) {

							return new CalCellRenderer(today, have_Appt);
						}
					} catch (Throwable e) {
//						System.exit(1);
					}
				...
				return new CalCellRenderer(null, have_Appt);

				// newly added start
				// String abc = edf[row][col];
				// if (tem.equals("") == false) {
				// try {
				// if (today.get(Calendar.YEAR) == currentY
				// && today.get(today.MONTH) + 1 == currentM
		...
		Appt[] sameLocationAppts = con.mApptStorage.RetrieveAppts(mCurrUser);
		for(Object key : con.mApptStorage.mLocations.keySet()){
			Location location = con.mApptStorage.mLocations.get(key);
			if(location.getStatus() > 0){
				for(int j = 0; j < sameLocationAppts.length; j++){
					if(sameLocationAppts[j].getLocation().getName().equals(location.getName())){
						RemoveLocationDialog rld = new RemoveLocationDialog(sameLocationAppts[j], location, this);
					}
				}
			}
		}
		
		Appt[] sameUserAppts = con.mApptStorage.RetrieveAppts(mCurrUser);
		if (sameUserAppts != null){
			for(int i = 0; i < us.getNumOfUsers(); i++){
				for(int j=0; j<sameUserAppts.length;j++){
					if(us.user[i].status > 0){
						if (!mCurrUser.ID().equals(us.user[i].ID())){
							if((sameUserAppts[j].getAttendList().contains(us.user[i]))||
	...
	// newly added start
	// public void getfreeArray(int[][] data) {
	// GregorianCalendar c = new GregorianCalendar(currentY, currentM - 1, 1);
	// int day;
	// int date;
	// Date d = c.getTime();
...
//			System.out.println("this user is noraml user.");
		...
		if (!mCurrUser.isAdmin()) {
			mi = (JMenuItem) Access.add(new JMenuItem("Change Information"));
			mi.setMnemonic('C');
			mi.getAccessibleContext().setAccessibleDescription(
					"For user logout");
			mi.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					CredentialDialog c = new CredentialDialog(mCurrUser);
					c.show();
				}
			});
			...
			mi = (JMenuItem) Access.add(new JMenuItem("Manage Accounts"));
			mi.setMnemonic('M');
			mi.getAccessibleContext().setAccessibleDescription(
						"For admin inspect");
			mi.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					AdminDialog d = new AdminDialog(mCurrUser, CalGrid.this);
					d.show();
				}
			});
			mi = (JMenuItem) Access.add(new JMenuItem("Manage Location"));
			mi.setMnemonic('L');
			mi.getAccessibleContext().setAccessibleDescription(
					"For admin inspect");
			mi.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					LocationsDialog d = new LocationsDialog("New", CalGrid.this);
					d.show();
				}
	...
	public void checkUpdateJoinAppt() {
		Appt[] EventWaitingList = controller.mApptStorage
				.RetrieveInviteAppts(getCurrUser());
		if (EventWaitingList == null) {
			return;
		}
		// System.out.println(EventWaitingList.length);
		for (int i = 0; i < EventWaitingList.length; i++) {
			JoinApptDialog A = new JoinApptDialog(
					EventWaitingList[i].getusername(), EventWaitingList[i],
					controller);
			A.show();
		}
	}
	
	public boolean check_Appt_On_day(int year, int month, int day) {
		boolean check = false;
		Timestamp start = new Timestamp(0);
		start.setYear(year);
		...
		TimeSpan K = new TimeSpan(start, end);
		Appt[] temp_check = controller.RetrieveAppts(mCurrUser, K);
